...
def load_materials_json():
    ...
    for _, row in materials_df.iterrows():
        ...
        tier = pd.to_numeric(row.get('Tier', 0), errors='coerce')
        catalog[ticker] = {
            'ticker': ticker,
            'name': row.get('Name', ''),
            ...
            'volume_per_unit': 0.0 if pd.isna(volume) else float(volume),
            'tier': 0.0 if pd.isna(tier) else float(tier),
        }
    return catalog


def load_material_volumes():
    """Load a compact material volume lookup table generated during the pipeline."""
    path = CACHE_DIR / CACHE_FILES['material_volumes']
    if path.exists():
        return pd.read_csv(path)

    materials_df = load_materials()
    if materials_df.empty:
        return pd.DataFrame(columns=['Ticker', 'Material Name', 'Volume per Unit'])

    volume_df = materials_df[['Ticker', 'Name', 'Volume']].copy()
