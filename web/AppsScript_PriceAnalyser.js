      ...
      if (recipe.includes('=>')) {
        ...
        if (outputPart) {
          const outputs = outputPart.split('-').map(item => {
            const match = item.match(/(\d+)x([A-Z0-9]+)/);
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      rows.push({
        ...
        volume: parseFloat(cell(row, ['Volume per Unit', 'Volume_per_Unit', 'Volume', 'volume', 'Volume (m3)', 'Volume m3'])) || 0,
        downstreamUses: parseInt(cell(row, ['Downstream Uses', 'InputCount']), 10) || 0,
        profitPerM3: parseFloat(cell(row, ['Profit per m3', 'Profit per m3 / Ship Volume', 'Profit_per_m3'])) || 0
      });
    }
...
function getExchanges() {
  ...
  // Skip header row, get exchanges from column E (4)
  for (let i = 1; i < data.length; i++) {
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][1] === material && data[i][4] === exchange && data[i][2]) {
        ...
        const askPrice = parseFloat(data[i][5]) || 0;        // Column F (5)
        let inputCostAsk = parseFloat(data[i][7]) || 0;      // Column H (7)
        let workforceCostAsk = parseFloat(data[i][9]) || 0;  // Column J (9)
...
function calculateSelfProductionCost(recipeString, allData, exchange, visited) {
  ...
  try {
    ...
    for (let inputStr of inputs) {
      const match = inputStr.match(/(\d+)x([A-Z0-9]+)/);
      if (match) {
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][1] === material && (!recipe || data[i][2] === recipe)) {
        const exch = data[i][4];  // Column E (4): Exchange
...
function getCalculationData(material, exchange, recipe, includeLuxury, selfProduced, planetName) {
  ...
  if (bestRow && bestRow[0] && normalizeString(bestRow[1]) === normMaterial) {
      ...
      const askPrice = parseFloat(data[i][5]) || 0;           // Column F (5): Ask_Price
      const bidPrice = parseFloat(data[i][6]) || 0;           // Column G (6): Bid_Price
      let inputCostAsk = parseFloat(data[i][7]) || 0;         // Column H (7): Input Cost Ask
      let inputCostBid = parseFloat(data[i][8]) || 0;         // Column I (8): Input Cost Bid
      ...
      let workforceCostAsk = parseFloat(data[i][9]) || 0;     // Column J (9): Workforce Cost Ask
      let workforceCostBid = parseFloat(data[i][10]) || 0;   // Column K (10): Workforce Cost Bid
      ...
      const supply = parseFloat(data[i][12]) || 0;            // Column M (12): Supply
      const demand = parseFloat(data[i][13]) || 0;            // Column N (13): Demand
      const traded = parseFloat(data[i][14]) || 0;            // Column O (14): Traded Volume
