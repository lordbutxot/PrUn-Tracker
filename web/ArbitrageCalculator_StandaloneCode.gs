...
function calculateArbitrage(originExchange, destExchange, minProfit, minROI, transportCostPerUnit) {
  try {
    ...
    function routeMatches(buyExchange, sellExchange) {
      if (originExchange && buyExchange !== originExchange) return false;
      if (destExchange && sellExchange !== destExchange) return false;
    ...
    function buildExchangeMap(sheet) {
      ...
      const idx = {
        ...
        volume: getVolumeColumnIndex(headers)
      };
    ...
    if (originSheet && destSheet) {
      ...
      if (section.found && section.rows.length > 0) {
        ...
        for (const row of section.rows) {
          if (!routeMatches(row.buyEx, row.sellEx)) continue;
      ...
      if (arbSheet) {
        ...
        for (let r = headerRowIndex + 1; r < data.length; r++) {
          ...
          if (!routeMatches(buyEx, sellEx)) continue;
      ...
      for (const ticker in priceMap) {
        ...
        for (const originCode of originList) {
          for (const destCode of destList) {
            ...
            const o = material.exchanges[originCode];
            const d = material.exchanges[destCode];
            if (!o || !d) continue;
